 ...
 */
export const zodArgument = (key, zod) => {
 ...
 */
export const zodOption = (key, zod) => {
 ...
 */
export const zodCommand = ({ name, description, args, opts, action, }) => {
    const command = new Command(name);
    if (description)
        command.description(description);
    for (const key in args)
        command.addArgument(zodArgument(key, args[key]));
    for (const key in opts)
        command.addOption(zodOption(key, opts[key]));
